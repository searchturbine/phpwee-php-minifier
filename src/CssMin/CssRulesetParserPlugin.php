...
/**
 * {@link CssParserPlugin Parser plugin} for parsing ruleset block with including declarations.
 *
 ...
 * @version        3.0.1
 */
class CssRulesetParserPlugin extends CssParserPlugin
{
    /**
    ...
    public function parse($index, $char, $previousChar, $state)
    {
        ...
        elseif ($char === ":" && $state === "T_RULESET_DECLARATION") {
            ...
            CssMin::triggerError(
                new CssError(
                    __FILE__,
                    __LINE__,
                    __METHOD__.": Unterminated declaration",
        ...
        elseif (($char === ";" || $char === "}") && $state === "T_RULESET_DECLARATION") {
            ...
            $this->parser->appendToken(
                new CssRulesetDeclarationToken(
                    $this->buffer,
                    $value,
                    $this->parser->getMediaTypes(),
                    $isImportant
                )
            );
