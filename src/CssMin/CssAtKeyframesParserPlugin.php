...
/**
 * {@link CssParserPlugin Parser plugin} for parsing @keyframes at-rule blocks, rulesets and declarations.
 *
 ...
 * @version        3.0.1
 */
class CssAtKeyframesParserPlugin extends CssParserPlugin
{
    /**
     * @var string Keyword
     */
    ...
    public function parse($index, $char, $previousChar, $state)
    {
        ...
        if ($char === "@"
            && $state === "T_DOCUMENT"
            && strtolower(substr($this->parser->getSource(), $index, 10)) === "@keyframes"
        ) {
        ...
        elseif ($char === "@"
            && $state === "T_DOCUMENT"
            && strtolower(substr($this->parser->getSource(), $index, 15)) === "@-moz-keyframes"
        ) {
        ...
        elseif ($char === "@"
            && $state === "T_DOCUMENT"
            && strtolower(substr($this->parser->getSource(), $index, 18)) === "@-webkit-keyframes"
        ) {
