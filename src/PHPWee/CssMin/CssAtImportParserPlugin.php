...
/**
 * {@link CssParserPlugin Parser plugin} for parsing @import at-rule.
 *
 ...
 * @version        3.0.1
 */
class CssAtImportParserPlugin extends CssParserPlugin
{
    /**
    ...
    public function parse($index, $char, $previousChar, $state)
    {
        if ($char === "@"
            && $state === "T_DOCUMENT"
            && strtolower(substr($this->parser->getSource(), $index, 7)) === "@import"
        ) {
            ...
            if ($pos) {
                ...
                CssMin::triggerError(
                    new CssError(
                        __FILE__,
                        __LINE__,
                        __METHOD__.": Invalid @import at-rule syntax",
